    ...
    App.contracts.SharedAssets.deployed().then(function(instance) {
      ...
      for (var i = 1; i <= assetCount; i++) {
        saInstance.assets(i).then(function(asset) {
          ...
          if (App.assetsShown[id]) return;
          
          App.assetsShown[id] = true;

          var timeStr = checkTime > 10 ? new Date(checkTime*1000).toLocaleString() : "-";

          var outBy = hasOut != 0 ? hasOut : "Item is In"
  
          // Render assets
          var assetTemplate = 
            "<tr><th>" + name + 
            "</th><td>" + (cost/(1e18)).toFixed(2) + 
            "</td><td>" + outBy + 
            "</td><td>" + timeStr + "</td></tr>";
